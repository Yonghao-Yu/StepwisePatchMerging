...
from spm import StepwisePatchMerging
...
class SPMWrapper(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        self.spm = StepwisePatchMerging(in_dim=in_chans, out_dim=embed_dim)

    def forward(self, x):
        B, C, H, W = x.shape

        x = self.spm(x.permute(0, 2, 3, 1)).flatten(1, 2)
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        for i in range(num_stages):
            if i == 0:
                ...
                patch_embed = SPMWrapper(img_size=img_size // (2 ** (i + 1)),
                                         patch_size=2,
